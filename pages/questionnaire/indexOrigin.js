...
Page({
  ...
  onShareAppMessage: function () {
    return {
      ...
      path: `/pages/questionnaire/index?id=${this.id}`,
  ...
  methods: {
    fetchData() {
      // api.getBannerList()
      api.getQuestionnaireDetail(this.id)
      .then(({ data }) => {
        console.log('问卷详情接口', data);
        // 模拟数据 start
        // data.data = {
        ...
        if (code === 200) {
          ...
          this.formData.map((item, index) => {
            ...
            if (item.field_type === 6) {
              // this.$set(this.otherStore, `${item.field_id}_other`, ' ');
              // this.otherStore[`${item.field_id}_other`] = ' ';
    ...
    otherRadioFocus(index, item) {
      // this.otherStore[`${item.field_id}_other`] =
      //   this.otherStore[`${item.field_id}_other`].replace(/(^\s*)|(\s*$)/g, '');

      // this.formList[index].value = this.otherStore[`${item.field_id}_other`];
    ...
    otherCheckboxFocus(index, item) {
      // let isPushOther =
      //   this.formList[index].value.some(item => item.type === 'other');

      // checked array 中 最后一个输入框所对应的只允许有一个
      // if(!isPushOther) {
      //   this.formList[index].value.push({
      //     type: 'other',
      //     value: this.otherStore[`${item.field_id}_other`],
      //   });
      // }
      ...
      let isPushOther = Boolean(this.formList[index].other);
    ...
    otherCheckboxInput(index, item) {
      // let otherIndex =
      //   this.formList[index].value.findIndex(item => item.type === 'other');

      // 更新
      // this.formList[index].value[otherIndex] = {
      //   type: 'other',
      //   value: this.otherStore[`${item.field_id}_other`],
      // }
      // this.formList[index].other = this.otherStore[`${item.field_id}_other`];
    ...
    handleSubmit() {
      ...
      submitData = submitData.map((item, index) => {
        ...
        return {
          ...
          //        item.value.filter(checkboxItem => Boolean(checkboxItem)) : item.value,
          value: item.value,
          other: item.other,
        }
      });
      ...
      api.getBannerList(this.id, {
        question_item: submitData,
      })
        .then(({ data }) => {
    ...
    getSignUpState() {
      return api.getSignUpState(this.id)
      .then(({ data }) => {
    ...
    getQuestionnaireSettings() {
      api.getQuestionnaireSettings(this.id)
      .then(({ data }) => {
